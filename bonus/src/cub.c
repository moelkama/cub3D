...
void	ft_put_vector1(t_info *info, void *img)
{
	...
	if (info->event_p == 0)
		mlx_put_image_to_window(info->mlx, info->window->win,
			info->weapon->w1->img, (info->window->width - 128) / 2,
			info->window->hight - 128);
	ft_putmini_map(info);
}
...
int	main(int c, char **v)
{
	...
	if (c == 2)
	{
		...
		mlx_hook(info->window->win, 6, 0, rotate_with_mouse, info);
		mlx_loop(info->mlx);
	}
	else
